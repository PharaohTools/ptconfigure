...
class ValidationHelpers {
    ...
    public function moreThan6Chars($fieldName, $options=array()) {
    ...
    public function lessThan10Chars($fieldName, $options=array()) {
    ...
    public function mustMatch($fieldName, $options=array() ) {
        $doesItMatch = ($options["fieldValue"] == $options["targetValue"])
            ? array("true"=>"") : array("false"=>"This field must match ".$options["targetFieldName"]) ;
        return $doesItMatch;
    }

    public function validUserLogin($fieldName, $options=array() ) {
        $user = new \Model\User();
        $validUser = ($user->attemptLogin($options["email"], $options["userPass"]) == true)
